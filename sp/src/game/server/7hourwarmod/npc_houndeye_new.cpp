...
void CNPC_Houndeye::Precache()
{
	...
	PrecacheParticleSystem("Houndeye_Shockwave_All");
...
void CNPC_Houndeye::SonicAttack ( void )
{
	...
	DispatchParticleEffect("Houndeye_Shockwave_All", GetAbsOrigin(), angles);
	...
	for (CEntitySphereQuery sphere(GetAbsOrigin(), HOUNDEYE_MAX_ATTACK_RADIUS); ( pEntity = sphere.GetCurrentEntity() ) !=NULL; sphere.NextEntity())
	{
		...
		if ( pEntity->m_takedamage != DAMAGE_NO || pPhysicsObject)
		{
			...
			if ( !FVisible( pEntity ) )
			{
				...
				else if ( !FClassnameIs( pEntity, "func_breakable" ) && !FClassnameIs( pEntity, "prop_physics" ) ) 
				{
...
void CNPC_Houndeye::StartTask( const Task_t *pTask )
{
	...
	case TASK_HOUND_GET_PATH_TO_CIRCLE:
	{
		...
		else 
		{
			...
			if (GetNavigator()->SetRadialGoal(vTargetPos, vec3_origin, random->RandomInt(50, 500), 90, 175, m_bLoopClockwise))
			{
				TaskComplete();
				return;
			}
