...
#define	GLOCK_FASTEST_REFIRE_TIME		0.05f
#define	GLOCK_FASTEST_DRY_REFIRE_TIME	0.1f
...
ConVar	glock_use_new_accuracy( "glock_use_new_accuracy", "0" );
...
void CWeaponGlock::ItemPostFrame( void )
{
	...
	if ( ( pOwner->m_nButtons & IN_ATTACK ) && ( m_flNextPrimaryAttack < gpGlobals->curtime ) && ( m_iClip1 <= 0 ) ) //previously else if
	{
		DryFire();
	}

	if ((GetIdealActivity() == ACT_VM_IDLE) && ( m_iClip1 <= 0 ))
	{
		SetIdealActivity(ACT_VM_IDLE_EMPTY);
	}
}
...
Activity CWeaponGlock::GetPrimaryAttackActivity( void )
{
	if (m_iClip1 <= 1)
	{
		//DevMsg("Last shot detected\n");
		m_bFiredLastShot = true;
		//SendWeaponAnim( ACT_GLOCK_FIRELAST );
		return ACT_GLOCK_FIRELAST;
	}
	else
	{
...
// Output : int
//-----------------------------------------------------------------------------
Activity CWeaponGlock::GetSecondaryAttackActivity(void)
{
	if (m_iClip1 <= 1)
	{
		//DevMsg("Last shot detected\n");
...
bool CWeaponGlock::Reload( void )
{
	if ( m_iClip1 <= 0) 
	{
		bool fRet = DefaultReload(GetMaxClip1(), GetMaxClip2(), ACT_GLOCK_RELOADEMPTY);
		if (fRet)
		{
			WeaponSound(RELOAD);
			m_flAccuracyPenalty = 0.0f;
			m_bFiredLastShot = false;
		}
		return fRet;
	}
	else
